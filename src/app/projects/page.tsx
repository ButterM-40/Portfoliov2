...
export default function ProjectsPage() {
  const { brand, projects } = usePortfolioData();
  ...
  const hero   = projects[featuredIdx] ?? projects[0];
  ...
  function selectFeatured(i: number) { setFeaturedIdx(i); setHeroSlide(0); setRoleOpen(false); beep(440, 0.08); window.scrollTo({ top: 0, behavior: 'smooth' }); }
  ...
  const accent = theme === 'midnight' ? '#FFDA14' : 'var(--accent)';
  ...
  return (
    <div data-theme={theme} style={{ fontFamily: "'JetBrains Mono',monospace", color: 'var(--ink)', background: 'var(--page)', minHeight: '100vh' }}>
        ...
        <div style={{ maxWidth: 1240, margin: '0 auto', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 14, background: 'var(--nav-bg)', backdropFilter: 'blur(14px)', border: '2px solid var(--border)', borderRadius: 999, padding: '10px 12px 10px 22px', boxShadow: '0 4px 0 var(--shadow)' }}>
          ...
          {isMobile ? (
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
              ...
              <span style={{ color: 'var(--ink)', fontSize: 12, fontWeight: 800, letterSpacing: 0.5, textTransform: 'uppercase', padding: '8px 14px', borderRadius: 999, background: 'rgba(255,218,20,0.12)', border: `1.5px solid ${accent}` }}>Projects</span>
        ...
        {isMobile && menuOpen && (
          <div style={{ maxWidth: 1240, margin: '8px auto 0', background: 'var(--nav-bg)', backdropFilter: 'blur(14px)', border: '2px solid var(--border)', borderRadius: 20, padding: '16px 20px', boxShadow: '0 6px 0 var(--shadow)', display: 'flex', flexDirection: 'column', gap: 4 }}>
            {[['/', 'Portfolio'], ['/games', 'Games'], ['/projects', 'Projects'], ['/research', 'Research']].map(([href, label]) => (
              <Link key={href} href={href} onClick={() => setMenuOpen(false)} style={{ textDecoration: 'none', color: href === '/projects' ? 'var(--accent)' : 'var(--ink)', fontSize: 15, fontWeight: 700, letterSpacing: 0.5, textTransform: 'uppercase', padding: '12px 10px', borderRadius: 12, display: 'block', borderBottom: '1px solid var(--border)' }}>{label}</Link>
            ))}
      ...
      {/* FEATURED HERO */}
      <section style={{ background: theme === 'midnight' ? '#0B0E1C' : 'var(--s-projects)', padding: isMobile ? '32px 16px 44px' : '44px 24px 60px', borderBottom: `1px solid ${theme === 'midnight' ? '#171B30' : 'var(--border)'}` }}>
        <div style={{ maxWidth: 1240, margin: '0 auto' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 28 }}>
            ...
            <span style={{ fontSize: 11, fontWeight: 700, letterSpacing: 2.5, textTransform: 'uppercase', color: theme === 'midnight' ? '#3A4A6A' : 'rgba(255,255,255,0.75)' }}>Featured Project</span>
          </div>
          <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : '1.15fr 0.95fr', gap: isMobile ? 24 : 48, alignItems: 'start' }}>
            {/* Carousel */}
            <div>
              <div style={{ position: 'relative', aspectRatio: '16/9', borderRadius: 12, overflow: 'hidden', border: `2px solid ${theme === 'midnight' ? '#232844' : 'rgba(255,255,255,0.4)'}`, boxShadow: '0 14px 0 rgba(0,0,0,0.3)', background: curYtId ? '#000' : (cur as any).image ? `url(${(cur as any).image}) center/cover no-repeat` : grad(cur?.grad ?? 'dark') }}>
                {curYtId ? (
                  <iframe
              ...
              <div style={{ display: 'flex', gap: 8, marginTop: 10 }}>
                {slides.map((sl, i) => {
                  ...
                  return (
                    <button key={i} onClick={() => { setHeroSlide(i); beep(400, 0.06); }} style={{ flex: 1, aspectRatio: '16/9', borderRadius: 7, overflow: 'hidden', border: i === heroSlide ? `2px solid ${accent}` : `2px solid ${theme === 'midnight' ? '#1E2240' : 'var(--border)'}`, background: slBg, cursor: 'pointer', padding: 0, position: 'relative' }}>
            ...
            <div style={{ paddingTop: 8 }}>
              <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', marginBottom: 18 }}>
                <span style={{ background: 'var(--chip-bg)', border: '1.5px solid var(--chip-border)', borderRadius: 999, padding: '5px 13px', fontSize: 11, fontWeight: 700, letterSpacing: 1, textTransform: 'uppercase', color: 'var(--card-soft)' }}>{hero?.engine}</span>
                <span style={{ background: 'var(--chip-bg)', border: '1.5px solid var(--chip-border)', borderRadius: 999, padding: '5px 13px', fontSize: 11, fontWeight: 700, letterSpacing: 1, textTransform: 'uppercase', color: 'var(--card-soft)' }}>{hero?.year}</span>
                <span style={{ background: 'rgba(255,218,20,0.1)', border: '1.5px solid rgba(255,218,20,0.3)', borderRadius: 999, padding: '5px 13px', fontSize: 11, fontWeight: 700, letterSpacing: 1, textTransform: 'uppercase', color: '#FFDA14' }}>{hero?.role}</span>
                <span style={{ background: 'rgba(46,230,197,0.1)', border: '1.5px solid rgba(46,230,197,0.3)', borderRadius: 999, padding: '5px 13px', fontSize: 11, fontWeight: 700, letterSpacing: 1, textTransform: 'uppercase', color: '#2EE6C5' }}>{hero?.tag}</span>
              </div>
              <h1 style={{ fontFamily: "'Fredoka',sans-serif", fontWeight: 700, fontSize: 56, lineHeight: 0.9, margin: 0, color: theme === 'midnight' ? '#EEF0FF' : '#fff', letterSpacing: -0.5 }}>{hero?.title}</h1>
              <div style={{ fontSize: 12, fontWeight: 700, letterSpacing: 1.5, textTransform: 'uppercase', color: accent, marginTop: 14 }}>{hero?.genre}</div>
              <p style={{ fontSize: 14, lineHeight: 1.82, color: theme === 'midnight' ? '#6878A0' : 'rgba(255,255,255,0.85)', margin: '16px 0 0', maxWidth: 460 }}>{hero?.desc}</p>
              {(hero as any)?.roleDesc && (
                <div style={{ marginTop: 16, border: `1.5px solid ${theme === 'midnight' ? '#1E2240' : 'rgba(255,255,255,0.3)'}`, borderRadius: 10, overflow: 'hidden' }}>
                  <button onClick={() => setRoleOpen(o => !o)} style={{ width: '100%', display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '11px 16px', background: 'rgba(0,0,0,0.2)', border: 'none', cursor: 'pointer', fontFamily: "'JetBrains Mono',monospace", fontSize: 11, fontWeight: 700, letterSpacing: 1.5, textTransform: 'uppercase', color: '#fff' }}>
                    <span>What I did</span>
                    <span style={{ color: accent, fontSize: 12 }}>{roleOpen ? '▲' : '▼'}</span>
                  </button>
                  {roleOpen && <div style={{ padding: '14px 16px', background: 'rgba(0,0,0,0.15)', borderTop: '1px solid rgba(255,255,255,0.15)' }}><p style={{ fontSize: 13, lineHeight: 1.8, color: 'rgba(255,255,255,0.8)', margin: 0 }}>{(hero as any).roleDesc}</p></div>}
                </div>
              )}
              <div style={{ marginTop: 24 }}>
                <div style={{ fontSize: 10, fontWeight: 700, letterSpacing: 2, textTransform: 'uppercase', color: theme === 'midnight' ? '#2A3450' : 'rgba(255,255,255,0.6)', marginBottom: 10 }}>Tech Stack</div>
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: 7 }}>
                  {hero?.tech.map(t => <span key={t} style={{ background: 'rgba(255,255,255,0.1)', border: '1.5px solid rgba(255,255,255,0.2)', borderRadius: 999, padding: '6px 13px', fontSize: 12, fontWeight: 600, color: '#fff' }}>{t}</span>)}
                </div>
              </div>
              <div style={{ display: 'flex', gap: 12, marginTop: 32, flexWrap: 'wrap' }}>
                <a href={hero?.link} target="_blank" rel="noopener noreferrer" style={{ textDecoration: 'none', background: accent, color: '#111827', border: '2px solid #111827', borderRadius: 999, padding: '14px 30px', fontSize: 13, fontWeight: 800, letterSpacing: 1, textTransform: 'uppercase', boxShadow: '0 5px 0 rgba(0,0,0,0.3)' }}>↗ View Live</a>
                <a href={hero?.repo} target="_blank" rel="noopener noreferrer" style={{ textDecoration: 'none', background: 'rgba(255,255,255,0.12)', color: '#fff', border: '2px solid rgba(255,255,255,0.3)', borderRadius: 999, padding: '14px 30px', fontSize: 13, fontWeight: 800, letterSpacing: 1, textTransform: 'uppercase', boxShadow: '0 5px 0 rgba(0,0,0,0.2)' }}>GitHub Repo</a>
              </div>
            </div>
          </div>
        </div>
      ...
      <section style={{ padding: '64px 24px 80px', background: 'var(--page)' }}>
        <div style={{ maxWidth: 1240, margin: '0 auto' }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 24, flexWrap: 'wrap', gap: 12 }}>
            <h2 style={{ fontFamily: "'Fredoka',sans-serif", fontWeight: 700, fontSize: 40, margin: 0, color: 'var(--ink)', textTransform: 'uppercase', letterSpacing: 0.5 }}>All Projects</h2>
            <span style={{ fontSize: 11, fontWeight: 700, letterSpacing: 2, textTransform: 'uppercase', color: 'var(--card-soft)' }}>{filtered.length} Projects</span>
          </div>
          <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', marginBottom: 36 }}>
            {allTags.map(t => (
              <button key={t} onClick={() => { setActiveTag(t); beep(500, 0.06); }} style={{ cursor: 'pointer', background: t === activeTag ? accent : 'var(--card)', color: t === activeTag ? '#111827' : 'var(--card-soft)', border: `1.5px solid ${t === activeTag ? '#111827' : 'var(--border)'}`, borderRadius: 999, padding: '9px 18px', fontFamily: "'JetBrains Mono',monospace", fontSize: 11, fontWeight: 700, letterSpacing: 0.5, textTransform: 'uppercase' }}>{t}</button>
            ))}
          </div>
          <div style={{ display: 'grid', gridTemplateColumns: isMobile ? 'repeat(2,1fr)' : 'repeat(3,1fr)', gap: 16 }}>
            {filtered.map(p => {
              const isFeatured = p === hero;
              ...
              return (
                <div key={p.id} onClick={() => selectFeatured(idx)} style={{ cursor: 'pointer', background: 'var(--card)', border: isFeatured ? `2px solid ${accent}` : '2px solid var(--border)', borderRadius: 14, overflow: 'hidden', boxShadow: isFeatured ? `0 6px 0 ${accent}44` : '0 6px 0 var(--shadow)' }}>
                  ...
                  <div style={{ padding: '14px 16px 16px' }}>
                    ...
                    <div style={{ fontSize: 11, color: 'var(--card-soft)', marginBottom: 12, letterSpacing: 0.5 }}>{p.genre}</div>
                    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                      <span style={{ fontSize: 10, fontWeight: 700, letterSpacing: 1, textTransform: 'uppercase', color: 'var(--card-soft)' }}>{p.engine} · {p.year}</span>
